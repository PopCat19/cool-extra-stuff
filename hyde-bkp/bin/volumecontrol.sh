...
# Check if SwayOSD is installed and running for on-screen volume display
use_swayosd=false
...
Optional:
    step  Volume change step (default: 4)

Examples:
    $(basename "$0") -o i 4     # Increase output volume by 4
    $(basename "$0") -i m       # Toggle input mute
    $(basename "$0") -p spotify d 4  # Decrease Spotify volume by 4 
    $(basename "$0") -p '' d 4  # Decrease volume by 4 for all players 

EOF
    exit 1
}

# Display volume notification with icon and volume bar
notify_vol() {
    # Round volume to nearest multiple of 5 for icon selection
    angle=$(( (($vol + 2) / 5) * 5 ))
    ico="${icodir}/vol-${angle}.svg"
    # Create a visual volume bar using dots
    bar=$(seq -s "." $(($vol / 15)) | sed 's/[0-9]//g')
    notify-send -a "t2" -r 91190 -t 800 -i "${ico}" "${vol}${bar}" "${nsink}"
}

# Display mute/unmute notification
notify_mute() {
...
# Change volume for different devices (pamixer or playerctl)
change_volume() {
    local action=$1
    local step=$2
    local device=$3
...
# Toggle mute for different devices
toggle_mute() {
    local device=$1
    local mode="--output-volume"
    [ "${srce}" = "--default-source" ] && mode="--input-volume"
...
# Toggle between available output devices
toggle_output() {
...
step=4  # Changed default volume step to 4
# Parse options
